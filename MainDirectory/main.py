...
while True:
    ...
    # evaluating the chosen models' performances on the test set and get the "best" classifiers prediction for the input
    for classifier in chosen_Classifiers:
        ...
        # save the the classifier with the maximum accuracy
        if accuracy > max_acc:
            max_acc = accuracy
    ...
    while True:
        X_user = preprocess_input(headline_input, vectorizer)
        print("Your headline: ", headline_input)
        input_classification = best_classifier_estimator.predict(X_user)
        print(best_classifier, " classifies your headline as: ",convert(input_classification))
        title = "another one?"
